...
from kalman import KalmanSingle, KalmanDelta, Ewma

import unittest

class KalmanTest(unittest.TestCase):
    def setUp(self):
        self.kd = KalmanDelta(t_init = -1)
        self.ks = KalmanSingle()

    def testKalmanDelta(self):
        """@TODO check for error in v[1]
        """
        self.kd = KalmanDelta(t_init = 0, 
                              x_init=array([[10.],[1/64.]],'f4'),
                              accel=1e-7, R=0.0025,
                              P_init = array([[0., 0.],[0.,0.]], 'f4'))
        sse = 0.
        for i in xrange(1,2001):
            z = ((i / 64.) + 10.)
            v = self.kd.filter(z, i)
            err = v[0] - z
