...
module EventDetectorTestP @safe()
{
  uses {
    interface Boot;
    interface TransmissionControl;
    interface Read<FilterState *> as EventRead;
  }
}
...
implementation
{
  ...
  event void EventRead.readDone(error_t result, FilterState* data) {
    //read in value from data
    FilterState transState;
    transState = *data;
    var = transState.x;
    test=result;
  }
