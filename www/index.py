...
def _splinePlot(typ, nid, t, v, dt, last_value, last_heard, last_delta, deltaTimeDict, startts, endts, debug, fmt, req, noData=False):
    ...
    try:
        ...
        (prev_time, prev_value, prev_delta) = _latest_reading_before(session, nid, typ, startts)
        ...
        gotPastDelta=False
        if prev_delta!=None and prev_value!=None:
            dtp=[prev_time]
            preTN=[matplotlib.dates.date2num(prev_time)]
            preV=[prev_value]
            gotPastDelta=True
        ...
        newDeltaTimeDict={}
        if prev_delta != None:
            newDeltaTimeDict[matplotlib.dates.date2num(prev_time)]=float(prev_delta)/300.
        for k,dv in deltaTimeDict.iteritems():
            newDeltaTimeDict[k]=float(dv)/300.
        deltaTimeDict=newDeltaTimeDict

        (last_time, last_value, last_delta) = _latest_reading_before(session, nid, typ, endts)

        duration = (endts - last_time).seconds
