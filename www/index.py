...
_sidebars = [
    ("Temperature", "allGraphs?typ=0",
     "Show temperature graphs for all nodes"),
    ...
    ("Electricity", "allGraphs?typ=40",
     "Show electricity usage for all nodes"),
    ...
    ("Missing and extra nodes", "missing",
     "Show unregistered nodes and missing nodes"),
...
def _nav():
    return ('<div id="nav">' +
            '<ul>' +
            ''.join(['<li><a href="%s" title="jump to %s">%s</a></li>'
                     % (b, a, a)
                     for (a, b) in _navs]) +
            '</ul>' +
            '</div>')
...
def _sidebar():
    return ('<div id="sidebar">' +
            '<ul>' +
            ''.join(['<li><a href="%s" title="%s">%s</a></li>' % (b, c, a)
...
def _row(alist, typ='d'):
    ...
    """
    return ('<tr>'+''.join(["<t{1}>{0}</t{1}>".format(x, typ) for x in alist])
            + '</tr>')
...
def _footer():
    return ('<div id="footer">CogentHouse &copy; '
...
def tree(req, period='day', debug=''):
    ...
    try:
        ...
        with p.stdin as dotfile:
            ...
            seen_nodes = set()
            qry = (session.query(NodeState.nodeId,
                                 Location.houseId,
                                 Room.name,
                                 NodeState.parent,
                          ...
                          .filter(and_(NodeState.time > t,
                                       NodeState.parent != 65535)))
            for (ni, hi, rm, pa, rssi) in qry:

                dotfile.write('{0}->{1} [label="{2}"];'
                              .format(ni, pa, float(rssi)))
...
def treePage(period='day'):
    ...
    s.append(_para(_href('<img src="' +
                         u +
                         '" alt="network tree diagram" width="700"/>',
                         u)))
    # s.append('<p>')
...
def humExpNodeGraph(node=None):
    try:
        ...
        (h, r) = (session.query(House.address, Room.name)
                  .join(Location, House, Room)
                  .filter(Node.id == int(node)).one())
        s = ['<p>']
...
def humExpGraph(req, node='64', debug=None, fmt='bo'):
    ...
    try:
        ...
        dryqry = (session.query(Reading.time, Reading.value)
                  .filter(
        ...
        comfortqry = (session.query(Reading.time, Reading.value)
                      .filter(
                          and_(Reading.nodeId == int(node),
                               Reading.typeId == 57))
                      .order_by(Reading.time))
        ...
        dampqry = (session.query(Reading.time, Reading.value)
                   .filter(
                       and_(Reading.nodeId == int(node),
                            Reading.typeId == 58))
                   .order_by(Reading.time))
        ...
        riskqry = (session.query(Reading.time, Reading.value)
                   .filter(
                       and_(Reading.nodeId == int(node),
        ...
        if not debug:
            ...
            if len(t) > 0:
                ...
                ax.plot_date(t, comfort, fmt, lw=2,
...
def humExposure():
    try:
        ...
        s = ['<p>']
        is_empty = True
        for (i, h, r) in (session.query(Node.id, House.address, Room.name)
                          .join(Location, House, Room)
                          .order_by(House.address, Room.name)):
            is_empty = False

            fr = (session.query(Reading)
                  .filter(and_(Reading.nodeId == i,
                               Reading.typeId == 57)).first())

            if fr is not None:
                u = _url("humExpNodeGraph", node=i)
                u2 = _url("humExpGraph", node=i)
                s.append('<p><a href="%s"><div id="grphtitle">%s'
                         '</div><img src="%s" alt=\"graph for node %d'
    ...
    try:
        ...
        if not debug:
            ...
            if len(t) > 0:
                ...
                ax.plot_date(t, comfort, fmt, lw=2,
                             linestyle="-", color=col[2])
                ax.plot_date(t, warm, fmt, lw=2, linestyle="-", color=col[3])
                ax.plot_date(t, over, fmt, lw=2,
                             linestyle="-", fillstyle="full", color=col[4])
    ...
    try:
        ...
        for (i, h, r) in (session.query(Node.id, House.address, Room.name)
            ...
            if fr is not None:
                ...
                s.append('<p><a href="%s"><div id="grphtitle">%s</div>'
...
def allGraphs(typ="0", period="day"):
    try:
        ...
        is_empty = True
        for (i, h, r) in (session.query(Node.id,
                                        House.address,
                                        Room.name)
                                 .join(Location, House, Room)
                                 .order_by(House.address, Room.name)):
    ...
    try:
        ...
        for h in session.query(House):
            ...
            u = _url("bathElecImg",
                     ...
                     duration=mins)
            s.append('<p><div id="grphtitle">%s</div>'
                     '<img src="%s" alt="bath / elec graph for %s" '
                     'width="700" height="400"></a></p>'
                     % (h.address, u, h.address))
...
def exportDataForm(err=None):
    ...
    try:
        ...
        s.append('<table border=\"0\" width=\"650\" '
                 'cellpadding=\"5\"><tr><td>')
        s.append('Start Date: <input type=\"text\" '
        ...
        s.append('End Date: <input type=\"text\" name=\"EndDate\" '
                 'value=\"' +
...
def getData(req, sensorType=None, StartDate=None, EndDate=None):
    ...
    try:
        ...
        exportData = (session.query(Reading.nodeId,
                                    Reading.time,
                                    Reading.value,
                                    House.address,
                                    Room.name)
...
def missing():
    try:
        ...
        report_set = set([int(x) for (x, ) in
                          session.query(distinct(NodeState.nodeId))
                          .filter(NodeState.time > t).all()])
        all_set = set([int(x) for (x, ) in
        ...
        if len(missing_set) == 0:
            ...
            s.append('</table><p><p><h3>Registered nodes '
                     'not reporting in last eight hours</h3><p>')

            s.append("<table border=\"1\">")
            s.append('<tr><th>Node</th><th>House</th>'
                     '<th>Room</th><th>Last Heard</th><th></th></tr>')

            qry = (session.query(func.max(NodeState.time),
                                 NodeState.nodeId,
                                 House.address,
                                 Room.name)
                          ...
                          .order_by(House.address, Room.name).all())
            for (maxtime, nodeId, house, room) in qry:
                u = _url("unregisterNode",
                         node=nodeId)
                s.append('<tr><td>%d</td><td>%s</td>'
                         '<td>%s</td><td>%s</td><td>'
...
def lastreport():
    try:
        ...
        for nid, maxtime in (session.query(NodeState.nodeId,
                                           func.max(NodeState.time))
                                    .group_by(NodeState.nodeId).all()):
...
def yield24():
    try:
        ...
        yield_q = (session.query(maxseq_q.c.nodeId,
                          ...
                          .join(minseq_q,
                                minseq_q.c.nodeId == maxseq_q.c.nodeId)
                          .join(seqcnt_q,
...
def dataYield():
    try:
        session = Session()
        s = []
        s.append('<h4>Note: this page does not yet '
                 'support SIP and the yield may be wrong.</h4>')
        s.append("<table border=\"1\">")
        s.append('<tr><th>Node</th>'
                 '<th>House</th>'
                 '<th>Room</th>'
                 '<th>Message Count</th>'
        ...
        for nid, cnt, mintime, maxtime in session.query(
            ...
            s.append('<tr><td>%d</td>'
                     ...
                     '<td>%s</td>'
                     '<td>%8.2f</td></tr>'
                     % (nid, house, room, cnt, mintime, maxtime, y))
...
def registerNode(node=None, room=None):
    try:
        ...
        s.append('<p>Node id: %d<input type="hidden" name="node" '
                 'value="%d"/></p>'
                 % (node, node))
...
def addNewHouse(regnode=None, err=None, address=None):
    ...
    try:
        ...
        s.append('<p>Address: <input type="text" name="address" '
                 'value="%s"/></p>'
                 % address)
...
def addNewHouseSubmit(regnode=None, address=None, deployment=None):
    try:
        ...
        if h is not None:
            return _redirect(_url("addNewHouse",
                                  regnode=regnode,
                                  address=address,
                                  err='duphouse'))
...
def addNewRoomSubmit(regnode=None, name=None, roomtype=None):
    try:
        ...
        if roomtype is None:
            return _redirect(_url("addNewRoom",
                                  regnode=regnode,
                                  name=name,
                                  err='nullroomtype'))
...
def addNewRoomType(ref=None, err=None, name=None):
    ...
    try:
        ...
        s.append('<p>Room type: <input type="text" name="name" '
                 'value="%s" /></p>'
                 % (name))
...
def addNewRoomTypeSubmit(ref=None, name=None):
    try:
        ...
        if len(name) > 20:
            name = name[:20]
        if len(name) < 3:
            return _redirect(_url("addNewRoomType",
                                  ref=ref,
                                  name=name,
                                  err='short'))
...
def nodesInHouseList(hid=None, sort='id'):
    try:
        ...
        (hname,) = (session.query(House.address)
                    .filter(House.id == int(hid))).first()

        h = (session.query(Node.id,
                           Room.name)
                    .join(Location, House, Room)
        ...
        s = []
        s.append('<table border="1">')
        s.append(_row([_href('id', _url('nodesInHouseList',
                                        hid=hid,
                                        sort='id')),
                       _href('room', _url('nodesInHouseList',
                                          hid=hid,
                                          sort='name'))], typ='h'))
        for hid, rname in h:
            u = _url("nodeGraph", node=hid)
            s.append(_row(['<a href="{url}">{id}</a>'.format(url=u, id=hid),
                           '<a href="{url}">{id}</a>'
                           .format(url=u, id=rname)]))
        s.append("</table>")
        return _page('Node list for {hname}'.format(hname=hname),
                     ''.join(s))
    finally:
        session.close()
        ...
        return _page('Flash node confirmation',
                     _form(_input(typ='hidden',
                           ...
                           _input(typ='hidden',
                                  name='device',
                                  value=device) +
                           _para('Confirm that you want to flash '
                                 'the node on {0} as a {1} with id {2}'
                                 .format(device, typ, idnum)),
                           action='flashNodeGo',
                           button='Yes, I know what I am doing'))
    else:
        return _redirect(_url('flashNodePage',
                              err='Node disappeared'))
