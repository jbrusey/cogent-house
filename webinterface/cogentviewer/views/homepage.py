...
def homepage(request):
    ...
    theQry = theQry.filter(models.NodeState.time > datetime.datetime.now() - datetime.timedelta(days=1))
    ...
    #theQry = theQry.filter(func.max(models.NodeState.time) <= datetime.datetime.now() - datetime.timedelta(days=1))
    theQry = theQry.having(func.max(models.NodeState.time) <= datetime.datetime.now() - datetime.timedelta(days=1))
    theQry = theQry.order_by(func.max(models.NodeState.time).desc())
