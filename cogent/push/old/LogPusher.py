...
class Pusher(object):
    ...
    def hash(self):
        log.debug("Testinh Hash Functions")
        session = self.LocalSession()
        theQry= session.query(models.SensorType).limit(3)
        for item in theQry:
            print item
            print hash(item)
